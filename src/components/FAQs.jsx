...
function FAQs() {
  ...
  return (
    <div className="py-16 text-white bg-[#0B1221] flex justify-center bg-[url('./assets/bg.svg')] bg-[length:100%] bg-no-repeat bg-left">
      <div className="bg-white bg-opacity-[0.03] p-6 md:p-10 rounded-xl w-full md:w-[60vw]">
        <h2 className="text-center text-3xl md:text-4xl font-bold mb-8 font-fontfira text-[#E4B40D]">
          FAQs
        </h2>
        <div className="max-w-full mx-auto font-fontRal text-lg md:text-xl">
          {faqs.map((faq, idx) => (
            <div key={idx} className="border-b border-gray-700 py-4">
              ...
              >
                ...
                <span className="text-2xl md:text-4xl">{open === idx ? "-" : "+"}</span>
              </div>
              <div
              ...
              >
                <p className="mt-2 text-sm md:text-base">{faq.answer}</p>
              </div>
            </div>
          ))}
        </div>
